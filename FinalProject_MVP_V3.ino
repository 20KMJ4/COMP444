  ...
  switch (state) {
    ...
    case 2:  // blue button was pressed
      if (laysStock > 0) {
        ...
        lcd.setCursor(0, 0);
        lcd.print("Lays selected $3");
        lcd.setCursor(0, 1);
        lcd.print("Confirm?");
        neededCoins = laysCost;
      } else {
...
void dispense() {
  ...
  analogWrite(PWMA, 35);
  delay(165);             // wanted to stop motor after 1 cycle but would need gearbox
  //to reduce the speed. Timing is also not consistent.
  //Used trial and error for getting speed and delay time

...
void loop() {
  if (gameStarted == false) {
    startSequence();
    //Serial.print("roundsToWin"); troubleshooting
    ...
    while (gameStarted == true) {
      ...
      if (pressedButton < 4) {
        ...
        if (pressedButton == buttonSequence[i]) {
          ...
          //Serial.println("LoserroundCounter:"); troubleshooting
      ...
      if (millis() - startTime > timeLimit) {
        //Serial.print(roundCounter); troubleshooting
        loseSequence();
        break;
      }
    }
  ...
  if (gameStarted == true) {
    ...
    if (roundCounter >= roundsToWin) {
      //Serial.println("WinnerroundCounter:"); troubleshooting
      //Serial.print(roundCounter);
      winSequence();
    }
    delay(500);
...
void startSequence() {
  ...
  roundsToWin = round(random(1, 16));
  //roundsToWin = round(random(1, 6)); easy testing

  //randomSeed(1); repeats same pattern every time for easy testing
